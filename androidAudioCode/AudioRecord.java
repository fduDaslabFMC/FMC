...
{
	//通过content provider获取使用AudioRecord应用的音频控制配置信息
	...
	//audioControlInfo为音频控制配置参数，从左到右依次为
	//高频信息控制 highFreqCtrl
	//声纹混淆控制 voicePrintCtrl
	//麦克风、扬声器并发控制 sodCtrl
	//默认值为000，即默认3个维度都不会进行处理，返回原始音频流
	private String audioControlInfo = "000";
    ...
    public AudioRecord(AudioAttributes attributes, AudioFormat format, int bufferSizeInBytes,
        ...
        //Log.d("Chen","AudioRecord————————AudioRecord()的native_setup初始化成功");

        mSampleRate = sampleRate[0];
        mSessionId = session[0];
        mState = STATE_INITIALIZED;

        //获取调用AudioRecord应用的音频控制信息
        if(null==myuri){
        	//修改了PackageInstallerActivity后，应用安装时将会跳转到音频控制配置信息页面，默认显示推荐设置，用户也可手动配置，配置信息以content provider的形式提供查询
            myuri=Uri.parse("content://com.hlq07.dfvcmodule.provider/data");
            Log.d("Chen","AudioRecord————————URI(myuri)初始化成功");
        }
        //暂时通过获取应用包名来进行音频控制配置信息查询
        try{
                myPackageName=ActivityThread.currentApplication().getPackageName();

                if(null==cursor){
                    cursor=ActivityThread.currentApplication().getContentResolver().query(myuri, 
                           new String[]{"permissions"},"pkg_name=?",new String[]{myPackageName},null);
					...
					audioControlInfo = stringBuilder.toString();//更新配置信息
					Log.d("Chen","AudioRecord-------------从数据库中读取的audioControlInfo的信息为："+audioControlInfo);
				}

		    }catch(Exception e){
                    e.printStackTrace();
                    //若无法获取应用包名或无法查询到某包名的控制配置，则暂时先不进行控制
                    audioControlInfo = "000";
                    Log.d("Chen","AudioRecord————————系统应用或未知应用无法获取包名，控制信息全部置0");
             }
    ...
    throws IllegalStateException {
        ...
        synchronized(mRecordingStateLock) {
            //开始录音之前将控制信息传入
            //借助添加的JNI方法传递参数
            native_setAudioControlConfiguration(audioControlInfo);
            Log.d("Chen","AudioRecord————————AudioRecord()的setAudioControlConfiguration运行成功，开始录音");
            if (native_start(MediaSyncEvent.SYNC_EVENT_NONE, 0) == SUCCESS) {
                handleFullVolumeRec(true);
                mRecordingState = RECORDSTATE_RECORDING;
                Log.d("Chen","AudioRecord————————AudioRecord.java，native_start返回成功消息");
            }
        }
    }
    ...
    throws IllegalStateException {
        ...
        synchronized(mRecordingStateLock) {
        	native_setAudioControlConfiguration(audioControlInfo);
             Log.d("Chen","AudioRecord————————AudioRecord()的startRecording(MediaSyncEvent syncEvent)");
    ...
    public int read(@NonNull byte[] audioData, int offsetInBytes, int sizeInBytes,
        ...
        Log.d("Chen","AudioRecord.java------------------------read bytes函数");
    ...
    */
    ...
    //--------------------
    //添加native方法，传递音频控制配置参数
    private native final void native_setAudioControlConfiguration(String audioControlConfiguration);
