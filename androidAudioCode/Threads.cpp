...
// ----------------------------------------------------------------------------
status_t AudioFlinger::RecordThread::setAudioControlConfiguration(String16 audioControlConfig){
	mAudioControlConfiguration = audioControlConfig;//私有变量mAudioControlConfiguration保存配置信息
    LOGD("Threads.cpp----------setAudioControlConfiguration，传入的参数为：%s",String8(mAudioControlConfiguration).string());
    return NO_ERROR;
}
// ----------------------------------------------------------------------------
...
reacquire_wakelock:
    ...
    for (;;) {
        ...
        if (mPipeSource != 0) {
            LOGD("mPipeSource read 方法");
            size_t framesToRead = mBufferSize / mFrameSize;
            framesToRead = min(mRsmpInFramesOA - rear, mRsmpInFramesP2 / 2);
            mInput->stream->setAudioControlConfiguration(String8(mAudioControlConfiguration));
        ...
        // otherwise use the HAL / AudioStreamIn directly
            ...
            size_t bytesRead;
            mInput->stream->setAudioControlConfiguration(String8(mAudioControlConfiguration));
            LOGD("Threads.cpp,  threadLoop,传递参数");
