...
status_t AudioRecord::setAudioControlConfiguration(String16 audioControlInfo){
    mAudioControlConfig = audioControlInfo ;

    LOGD("AudioRecord.cpp--------------setAudioControlConfiguration()，传入的值为:%s",String8(mAudioControlConfig).string());
    return NO_ERROR;
}
...
ssize_t AudioRecord::read(void* buffer, size_t userSize, bool blocking)
{
    ...
    const sp<IAudioFlinger>& audioFlinger = AudioSystem::get_audio_flinger();//获取当前AudioFlinger提供的参数传递接口对象
    if (audioFlinger == 0) {
        LOGD("AudioRecord.cpp--------------获取audioFlinger（IAudioFlinger object）失败");
        return BAD_VALUE;
    }else{
        audioFlinger->setAudioControlConfiguration(mAudioControlConfig);//调用audioFlinger提供的接口
        LOGD("AudioRecord.cpp--------------read，传入的值为:%s，audioFlinger->setAudioControlConfiguration()成功返回",String8(mAudioControlConfig).string());
    }
