...
#define myTAG "IAudioFlinger_Chen"
...
public:
    ...
    virtual  status_t  setAudioControlConfiguration(String16 audioControlConfig){
        LOGD("IAudioFlinger----------setAudioControlConfiguration函数");
        Parcel data, reply;
        data.writeInterfaceToken(IAudioFlinger::getInterfaceDescriptor());
        data.writeString16(audioControlConfig);
        return remote()->transact(SET_AUDIO_CONTROL_CONFIG, data, &reply);
    }
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    ...
    switch (code) {
        ...
        case SYSTEM_READY: {
          ...
          case SET_AUDIO_CONTROL_CONFIG: {
            CHECK_INTERFACE(IAudioFlinger, data, reply);
            String16 config(data.readString16());
            status_t status = setAudioControlConfiguration(config);
            reply->writeInt32(status);
            return NO_ERROR;
        } break;
