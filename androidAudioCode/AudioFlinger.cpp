...
#define myTAG "AudioFlinger_Chen"
...
// ----------------------------------------------------------------------------
status_t AudioFlinger::setAudioControlConfiguration(String16 config)
{
   
    for (size_t i = 0; i < mRecordThreads.size(); i++) {
        mRecordThreads.valueAt(i)->setAudioControlConfiguration(config);
        LOGD("AudioFlinger.cpp----------mRecordThreads.valueAt(i)");
    }
    LOGD("AudioFlinger.cpp----------setAudioControlConfiguration，传入的参数为：%s",String8(config).string());
    return NO_ERROR;
}
