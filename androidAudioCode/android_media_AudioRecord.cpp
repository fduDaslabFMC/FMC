...
static String16 audioControlInfo("000");
...
// ----------------------------------------------------------------------------
static void android_media_AudioRecord_setAudioControlConfiguration(JNIEnv *env,jobject thiz, jstring audioControlConfiguration){
	ScopedUtfChars audioControlConfigurationStr(env, audioControlConfiguration);//jstring的解析
    audioControlInfo.setTo(String16(audioControlConfigurationStr.c_str()));//audioControlInfo为设置的全局变量，默认值为000
    LOGD("JNI AudioRecord----------------------android_media_AudioRecord_setAudioControlConfiguration,传入的控制信息为:%s",String8(audioControlInfo).string());
...
static jint android_media_AudioRecord_readInArray(JNIEnv *env,  jobject thiz,
    ...
    //在此处进行参数传递
    lpRecorder->setAudioControlConfiguration(audioControlInfo);
    LOGD("JNI AudioRecord----------------------android_media_AudioRecord_readInArray,传入的控制信息为:%s",String8(audioControlInfo).string());
...
static const JNINativeMethod gMethods[] = {
    ...
    //自定义添加的JNI方法
    {"native_setAudioControlConfiguration", "(Ljava/lang/String;)V" ,(void *)android_media_AudioRecord_setAudioControlConfiguration},
